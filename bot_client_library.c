			...
			while(SDL_PollEvent(&event)) {
				...
				switch(event.type) {
					...
					//case(SDL_MOUSEBUTTONDOWN): {
						...
						//----------CHOOSE 2 RANDOM NUMBERS: card_x_aux, card_y_aux--------------------------
						...
						if(waiting != 0) // If player is waiting for something.
							...
							else if((game == 1) && (now - start_time > 2)) { // If game is active and it has been more than 2 seconds since the last pick (bot plays every 2 seconds).
