...
typedef struct play_response {
	int code; // Código que reprensenta a fase da jogada e do jogo.
		...
		// 4 - Foi feita a 2ª escolha mas as cartas escolhidas são diferentes.
...
char *get_str2send(int i, int j, int r, int g, int b, int board_piece_code); // Devolve, segundo coordenadas dadas, a string de um dado lugar do tabuleiro.